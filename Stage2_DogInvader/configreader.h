...
// defining each section within config file
enum ConfigSection{SpaceShip, Aliens};
...
public:
    ...
    ConfigReader();
    ~ConfigReader();
    void setup(QString fileName, QString filePath, std::vector<std::tuple<SwarmComponent*, std::string, unsigned>> &aliens, AlienFactory &alienFactory);
    /************************** GETTERS ***********************/
...
private:
    ...
    void fixName();
    void fixMap();
    void extractSpaceShipConfig(std::string str);
    int extractAliensConfig(std::string str, std::vector<std::tuple<SwarmComponent*, std::string, unsigned>> &aliens, AlienFactory &alienFactory);
    void writeFile(bool usingDefault = false);
    void createDefault();

    /********** VARIABLES *********/
    QString path;
    std::map<std::string, std::string> spaceShipConfig;
    std::vector<std::string> aliensConfig;
    int coord;
    const std::vector<std::string> colors;
    //this is the default possible keys, values and aliens
    const std::vector<std::string> defaultKeys;
    const std::vector<std::string> defaultValues;
    const std::vector<std::string> defaultAliens;
